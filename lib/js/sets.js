...
$(document).ready(() => {
  ...
  $("#calculate").click(() => {
    ...
    function subtraction(a, b) {
      ...
      const aFreq = {};
      a.forEach((num) => {
        aFreq[num] = (aFreq[num] || 0) + 1;
      });

      // Count the frequency of each number in array 'b'
      const bFreq = {};
      b.forEach((num) => {
        bFreq[num] = (bFreq[num] || 0) + 1;
      });

      // Calculate the difference between the frequencies of each number in array 'a' and 'b'
      for (let num in aFreq) {
        const aCount = aFreq[num];
        const bCount = bFreq[num] || 0;
        const diff = aCount - bCount;

        // If the difference is positive, add the number to the result array that many times
        if (diff > 0) {
